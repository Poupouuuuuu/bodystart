...
export default function BundleComposite({
  ...
  // card : largeur égale par pot (flex-1) pour remplir la carte.
  // thumb : hauteur fixe (les pots restent lisibles dans 80px).
  const imgClass = isThumb
    ? 'w-auto h-[72%] object-contain drop-shadow-sm'
    : 'flex-1 w-0 min-w-0 h-auto object-contain drop-shadow-2xl'

  const intrinsic = isThumb ? 72 : 260

  return (
    <div
      className={`relative bg-cover bg-bottom bg-no-repeat overflow-hidden ${className}`}
      style={{ backgroundImage: "url('/Background.webp')" }}
    >
      {n > 0 ? (
        <div className={`absolute inset-0 flex items-end justify-center ${padClass}`}>
          <div
            className={`flex items-end justify-center ${isThumb ? '' : 'w-full'} ${overlap} ${
