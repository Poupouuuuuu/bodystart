import type { BlogArticle } from '@/lib/blog'

// Article SEO/GEO — cible « compléments après 40 ans / musculation 40 ans ».
// Allégations UE 1924/2006 uniquement (protéines = maintien masse musculaire ;
// vitamine D = os/immunité/fonction musculaire ; magnésium = fatigue/fonction
// musculaire). AUCUNE allégation de prévention de maladie (sarcopénie,
// ostéoporose = interdit). Oméga-3 mentionné comme nutriment (produit archivé
// → non lié). Handles + prix vérifiés Shopify 2026-07-17.
export const complementsApres40Ans: BlogArticle = {
  slug: 'complements-apres-40-ans',
  title: 'Compléments alimentaires après 40 ans : par où commencer ?',
  metaTitle: 'Compléments après 40 ans : par où commencer ?',
  metaDescription:
    'Quels compléments après 40 ans ? Protéines, vitamine D, magnésium, créatine : l\'ordre logique pour rester en forme, sans se ruiner ni survendre.',
  excerpt:
    "Après 40 ans, les priorités changent : les protéines pour préserver le muscle, la vitamine D et le magnésium comme fondations, la créatine pour la force. On te donne l'ordre logique, avec des allégations honnêtes — un complément à la fois, choisi pour une vraie raison.",
  datePublished: '2026-07-17',
  dateModified: '2026-07-17',
  sections: [
    {
      h2: 'Qu\'est-ce qui change vraiment après 40 ans ?',
      blocks: [
        {
          type: 'p',
          text: "Passé 40 ans, le corps ne s'effondre pas du jour au lendemain — mais quelques tendances de fond s'installent. La masse musculaire a naturellement tendance à diminuer avec l'âge quand on ne l'entretient pas, la récupération demande un peu plus de soin, et le statut en vitamine D est souvent bas, surtout en hiver. Rien d'alarmant : ce sont des raisons de structurer intelligemment son alimentation et, si besoin, sa supplémentation.",
        },
        {
          type: 'p',
          text: "La bonne nouvelle, c'est que les leviers qui comptent sont simples et connus. Inutile d'empiler quinze pots : quelques fondations bien choisies couvrent l'essentiel. On te les donne dans l'ordre de priorité, du plus utile au plus accessoire.",
        },
        {
          type: 'p',
          text: 'Un principe avant tout : **un complément ne remplace jamais une alimentation variée, l\'activité physique et le sommeil**. La supplémentation vient en complément d\'une hygiène de vie, pas à sa place.',
        },
      ],
    },
    {
      h2: 'Priorité n°1 : les protéines pour préserver le muscle',
      blocks: [
        {
          type: 'p',
          text: "S'il ne fallait retenir qu'un point, ce serait celui-là. Les **protéines contribuent au maintien de la masse musculaire** : c'est une allégation officiellement reconnue, et c'est exactement l'enjeu après 40 ans. Le muscle, ça s'entretient — par l'entraînement en résistance et par un apport suffisant en protéines.",
        },
        {
          type: 'p',
          text: 'Beaucoup de personnes de plus de 40 ans mangent, sans le savoir, un peu léger en protéines au petit-déjeuner et au goûter. Une whey est alors le moyen le plus simple d\'atteindre son quota sans se forcer : un shaker rapide comble le trou. On a détaillé les apports recommandés dans notre article [combien de protéines par jour](/blog/combien-de-proteines-par-jour).',
        },
        {
          type: 'p',
          text: 'Un choix fiable et abordable : la [Whey Native Protimuscle](/products/whey-native-protimuscle), une whey native au bon rapport qualité-prix. Si tu digères mal le lactose, oriente-toi vers une isolate. Le rayon complet est ici : [protéines](/categories/proteines).',
        },
      ],
    },
    {
      h2: 'La vitamine D : la fondation, surtout d\'octobre à mars',
      blocks: [
        {
          type: 'p',
          text: "La vitamine D est la carence la plus fréquente sous nos latitudes, et elle coche beaucoup de cases utiles avec l'âge. Officiellement, la **vitamine D contribue au maintien d'une ossature normale, à une fonction musculaire normale et au fonctionnement normal du système immunitaire**. Trois piliers qui parlent à tout le monde après 40 ans.",
        },
        {
          type: 'p',
          text: 'La logique est simple : peu d\'exposition au soleil d\'octobre à mars = supplémentation qui fait sens sur cette période. Deux formats dans notre sélection : la [Vitamine D3 + K2](/products/vitamin-d3-k2) en softgels (le duo D3-K2 est apprécié pour l\'os) et la [Vitamine D3 en gouttes](/products/vitamine-d3-gouttes), pratique et économique.',
        },
      ],
    },
    {
      h2: 'Le magnésium : fatigue, muscles et système nerveux',
      blocks: [
        {
          type: 'p',
          text: "Le magnésium est le deuxième réflexe fondation. Il **contribue à réduire la fatigue, à une fonction musculaire normale et au fonctionnement normal du système nerveux** — des allégations autorisées qui résument bien pourquoi on le conseille souvent aux personnes actives qui se sentent « à plat ».",
        },
        {
          type: 'p',
          text: 'Pour le confort digestif, privilégie une forme bien tolérée comme le **bisglycinate**, à l\'inverse de l\'oxyde de magnésium bon marché qui laisse plus souvent des désagréments. Notre [Magnésium Bisglycinate + B6](/products/magnesium-bisglycinate-vitamine-b6-taurine) coche cette case (27,90 €). Le magnésium se prend volontiers le soir, dans le cadre d\'une routine de fin de journée.',
        },
      ],
    },
    {
      h2: 'La créatine n\'est pas réservée aux jeunes',
      blocks: [
        {
          type: 'p',
          text: "On l'associe à tort aux vingtenaires en salle. Pourtant, la créatine est un des rares compléments dont l'intérêt est justement bien documenté chez les personnes actives qui prennent de l'âge, en soutien de la force. Rappel de l'allégation autorisée : la créatine **améliore les capacités physiques lors de séries successives d'exercices très intenses et de courte durée**.",
        },
        {
          type: 'p',
          text: 'Même dosage à tout âge : **3 à 5 g de monohydrate par jour**, tous les jours. Associée à un entraînement en résistance, c\'est un excellent partenaire de la force. Le rayon [créatine](/categories/creatine) rassemble nos formats ; le protocole précis est dans notre article [créatine avant ou après la séance](/blog/creatine-avant-ou-apres-seance).',
        },
      ],
    },
    {
      h2: 'Articulations, peau, cœur : les compléments « confort »',
      blocks: [
        {
          type: 'p',
          text: "Au-delà des fondations, certains produits répondent à des envies précises. Le **collagène** est populaire pour la peau et les articulations : c'est un produit plaisir et entretien, à choisir pour sa composition. À noter qu'officiellement, c'est la **vitamine C qui contribue à la formation normale de collagène** — d'où l'intérêt des formules qui associent les deux, comme le [Collagen Complex + Vitamine C](/products/collagen-complex-vitamine-c) ou le [Collagène Marin Liquide](/products/pure-collagen-marin-liquide).",
        },
        {
          type: 'p',
          text: 'Côté cœur, les **oméga-3 (EPA/DHA) contribuent à une fonction cardiaque normale** : intéressant si tu manges peu de poisson gras. On les range dans le rayon [santé & bien-être](/categories/sante), avec les vitamines et minéraux. Là encore, la règle est : un produit à la fois, choisi pour une raison précise, pas une pile de pots « au cas où ».',
        },
        {
          type: 'steps',
          items: [
            'Assure d\'abord ton apport en protéines (whey si l\'assiette ne suffit pas).',
            'Ajoute la vitamine D, surtout d\'octobre à mars.',
            'Ajoute le magnésium si tu ressens de la fatigue ou pour le confort du soir.',
            'Intègre la créatine si tu fais de la musculation ou des efforts intenses.',
            'Vois ensuite les « confort » (collagène, oméga-3) selon tes envies.',
          ],
        },
        {
          type: 'p',
          text: 'Tu veux qu\'on adapte tout ça à ton cas ? Le conseil est gratuit en boutique à Coignières — on te construit une routine simple, sans te vendre dix produits.',
        },
      ],
    },
  ],
  faq: [
    {
      q: 'Quels compléments prendre après 40 ans ?',
      a: 'Dans l\'ordre de priorité : des protéines pour préserver le muscle (une whey si l\'alimentation ne suffit pas), de la vitamine D surtout en hiver, du magnésium en cas de fatigue, et de la créatine si tu fais des efforts intenses. Le reste (collagène, oméga-3) vient après, selon tes envies.',
    },
    {
      q: 'Peut-on prendre de la whey à 40 ou 50 ans ?',
      a: 'Oui, sans problème pour une personne en bonne santé. La whey n\'est qu\'une source de protéines pratique ; les protéines contribuent au maintien de la masse musculaire, un enjeu qui devient justement plus important avec l\'âge. Elle complète l\'alimentation quand l\'assiette est un peu légère en protéines.',
    },
    {
      q: 'La créatine est-elle utile passé 40 ans ?',
      a: 'Oui. La créatine améliore les capacités physiques lors de séries d\'exercices intenses et courts, à tout âge, avec le même dosage de 3 à 5 g par jour. Associée à un entraînement en résistance, c\'est un bon soutien de la force chez les personnes actives qui prennent de l\'âge.',
    },
    {
      q: 'Faut-il prendre de la vitamine D toute l\'année ?',
      a: 'La supplémentation fait surtout sens d\'octobre à mars, quand l\'exposition au soleil est faible sous nos latitudes. La vitamine D contribue au maintien d\'une ossature normale, à une fonction musculaire normale et au fonctionnement normal du système immunitaire. En cas de doute sur ton statut, un dosage sanguin et l\'avis de ton médecin permettent d\'ajuster.',
    },
    {
      q: 'Le collagène est-il efficace pour la peau et les articulations ?',
      a: 'Le collagène est un produit d\'entretien apprécié pour la peau et les articulations, à choisir pour sa composition. Sur le plan réglementaire, c\'est la vitamine C qui contribue à la formation normale de collagène — d\'où l\'intérêt des formules qui associent collagène et vitamine C.',
    },
  ],
  products: [
    { handle: 'whey-native-protimuscle', label: 'Whey Native Protimuscle' },
    { handle: 'vitamin-d3-k2', label: 'Vitamine D3 + K2' },
    { handle: 'magnesium-bisglycinate-vitamine-b6-taurine', label: 'Magnésium Bisglycinate + B6' },
  ],
  categories: [
    { slug: 'sante', label: 'Santé & bien-être' },
    { slug: 'proteines', label: 'Protéines' },
  ],
  related: ['complements-debutant-musculation', 'combien-de-proteines-par-jour'],
}
